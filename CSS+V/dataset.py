...
class VQAFeatureDataset(Dataset):
    ...
    def tokenize(self, max_length=14):
        ...
        for entry in tqdm(self.entries, ncols=100, desc="tokenize"):
            ...
            qttokens = self.dictionary.tokenize(entry['question_type'], False)

            tokens = tokens[:max_length]
            qttokens = tokens[:max_length]
            ...
            # -----------------------------------------
            if len(qttokens) < max_length:
                # Note here we pad in front of the sentence
                padding = [self.dictionary.padding_idx] * (max_length - len(qttokens))
                padding_mask = [self.dictionary.padding_idx - 1] * (max_length - len(qttokens))
                tokens_mask = padding_mask + qttokens
                qttokens = padding + qttokens

            utils.assert_eq(len(qttokens), max_length)

            entry['qt_token'] = qttokens
    ...
    def tensorize(self):
        for entry in tqdm(self.entries, ncols=100, desc="tensorize"):
            ...
            questiont = torch.from_numpy(np.array(entry['qt_token']))
            question_mask = torch.from_numpy(np.array(entry['q_token_mask']))

            entry['q_token'] = question
            entry['qt_token'] = questiont
    ...
    def __getitem__(self, index):
        ...
        if self.name=='train':
            ...
            if "bias" in entry:
                return features, ques, target,entry["bias"],train_hint,type_mask,notype_mask,ques_mask,qtype

            else:
                return features, ques,target, 0,train_hint,qtype
        else:
