...
def train(model, train_loader, eval_loader,args,qid2type):
    ...
    for epoch in range(num_epochs):
        ...
        t = time.time()
        for i, (v, q, a, b, hintscore,type_mask,notype_mask,q_mask,qtype) in tqdm(enumerate(train_loader), ncols=100,
            ...
            if mode=='updn':
                pred, loss,_ = model(v, q, a, b, None, qtype)
                if (loss != loss).any():
                ...
                ## first train
                pred, loss,word_emb = model(v, q, a, b, None, qtype)
                ...
                pred, _, _ = model(v, q2, None, b, None, qtype)
                ...
                pred, loss, _ = model(v, q3, a2, b, None, qtype)
                ...
                pred, _, _ = model(v, q, None, b, v_mask, qtype)
                ...
                pred, loss, _ = model(v, q, a2, b, v_mask, qtype)
                ...
                if random_num<=qvp:
                    ## first train
                    # pred, loss, word_emb = model(v, q, a, b, None, qtype)
                    pred, predn, loss, word_emb = model(v, q, a, b, None, qtype)
                    if epoch < 0:
                      loss = loss
                    else:
                      ssloss = compute_self_loss(predn, a)
                      loss = loss + 20*ssloss
                    word_grad = torch.autograd.grad((pred * (a > 0).float()).sum(), word_emb, create_graph=True)[0]
                    ...
                    pred, _, _, _ = model(v, q2, None, b, None, qtype)
                    ...
                    pred, _, loss, _ = model(v, q3, a2, b, None, qtype)
                    if (loss != loss).any():
                        raise ValueError("NaN loss")
                    loss.backward()
                    nn.utils.clip_grad_norm_(model.parameters(), 0.25)
                    ...
                    pred, _, _, _ = model(v, q, None, b, v_mask, qtype)
                    pred_ind = torch.argsort(pred, 1, descending=True)[:, :5]
                    false_ans = torch.ones(pred.shape[0], pred.shape[1]).cuda()
                    false_ans.scatter_(1, pred_ind, 0)
                    a2 = a * false_ans
                    ...
                    pred, _, loss, _ = model(v, q, a2, b, v_mask, qtype)
...
def evaluate(model, dataloader, qid2type):
    ...
    for v, q, a, b, qids, _ in tqdm(dataloader, ncols=100, total=len(dataloader), desc="eval"):
        ...
        pred, _, _, _ = model(v, q, None, None, None, None)
